...
u32 CheckMoveLimitations(u32 battler, u8 unusableMoves, u16 check)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        // Gorilla Tactics
            ...
            unusableMoves |= 1u << i;
        // Star Pupil
        else if (check & MOVE_LIMITATION_CHOICE_ITEM && GetBattlerAbility(battler) == ABILITY_STAR_PUPIL && *choicedMove != MOVE_NONE && *choicedMove != MOVE_UNAVAILABLE && *choicedMove != move)
            unusableMoves |= 1u << i;
...
static enum MoveCanceler CancelerChoiceLock(struct BattleContext *ctx)
{
    ...
    if (gChosenMove != MOVE_STRUGGLE
     ...
     && (IsHoldEffectChoice(holdEffect) || ctx->abilities[ctx->battlerAtk] == ABILITY_GORILLA_TACTICS || ctx->abilities[ctx->battlerAtk] == ABILITY_STAR_PUPIL))
...
bool32 CanAbilityAbsorbMove(u32 battlerAtk, u32 battlerDef, enum Ability abilityDef, u32 move, enum Type moveType, enum FunctionCallOption option)
{
    ...
    case ABILITY_PERMAFROST:
        if (B_REDIRECT_ABILITY_IMMUNITY >= GEN_5 && moveType == TYPE_FIRE && GetBattlerMoveTargetType(battlerAtk, move) != MOVE_TARGET_ALL_BATTLERS)
        {
            effect = MOVE_ABSORBED_BY_STAT_INCREASE_ABILITY;
            statId = STAT_SPEED;
        }
...
u32 AbilityBattleEffects(enum AbilityEffect caseID, u32 battler, enum Ability ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_PSYCHIC_SURGE:
            ...
            break;
        case ABILITY_DIMENSION_TWISTER:
            if (TryChangeBattleTerrain(battler, STATUS_FIELD_TRICK_ROOM))
            {
                BattleScriptPushCursorAndCallback(BattleScript_DimensionTwisterActivates);
                effect++;
            }
            break;
